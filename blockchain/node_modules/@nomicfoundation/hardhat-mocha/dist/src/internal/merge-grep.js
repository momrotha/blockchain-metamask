 ...
 */
export function resolveMochaGrepFilter(grep, grepExclude, config) {
    ...
    if (include !== undefined && configFgrep !== undefined) {
        throw new HardhatError(HardhatError.ERRORS.HARDHAT_MOCHA.GENERAL.GREP_INCOMPATIBLE_OPTION);
    }
    ...
    if (configFgrep !== undefined) {
        throw new HardhatError(HardhatError.ERRORS.HARDHAT_MOCHA.GENERAL
            .GREP_EXCLUDE_INCOMPATIBLE_OPTION, { option: "fgrep" });
    }
    if (config.invert === true) {
        throw new HardhatError(HardhatError.ERRORS.HARDHAT_MOCHA.GENERAL
            .GREP_EXCLUDE_INCOMPATIBLE_OPTION, { option: "invert" });
    }
    ...
    if (include === undefined &&
        ...
        rejectUnsupportedPattern(includeName, String(config.grep), "meaning-changing regex flags (e.g. /i)");
    }
    // A CLI --grep takes precedence over the config's grep when present.
    const configGrep = typeof config.grep === "string" ? config.grep : config.grep?.source;
    ...
    const mergedHasNamedGroup = analyzePattern(effectiveInclude ?? "", false).hasNamedGroup ||
        analyzePattern(exclude, false).hasNamedGroup;
    const includeInfo = analyzePattern(effectiveInclude ?? "", mergedHasNamedGroup);
    const excludeInfo = analyzePattern(exclude, mergedHasNamedGroup);
    const totalCapturingGroups = includeInfo.capturingGroups + excludeInfo.capturingGroups;
    ...
    if (includeInfo.hasEscapedName) {
        rejectUnsupportedPattern(includeName, effectiveInclude ?? "", "an escaped group name (e.g. \\u0067)");
    }
    if (excludeInfo.hasEscapedName) {
        rejectUnsupportedPattern("--grep-exclude", exclude, "an escaped group name (e.g. \\u0067)");
    }
    ...
    if (includeInfo.numberedBackreferences.some((n) => n > includeInfo.capturingGroups && n <= totalCapturingGroups)) {
        rejectUnsupportedPattern(includeName, effectiveInclude ?? "", "a numbered backreference (e.g. \\1)");
    }
    ...
    if (includeInfo.capturingGroups > 0 &&
        excludeInfo.numberedBackreferences.some((n) => n <= totalCapturingGroups)) {
        rejectUnsupportedPattern("--grep-exclude", exclude, "a numbered backreference (e.g. \\1)");
    }
    ...
    if (includeInfo.namedBackreferences.some((name) => !includeInfo.groupNames.has(name) && excludeInfo.groupNames.has(name))) {
        rejectUnsupportedPattern(includeName, effectiveInclude ?? "", "a named backreference (e.g. \\k<name>)");
    }
    if (excludeInfo.namedBackreferences.some((name) => !excludeInfo.groupNames.has(name) && includeInfo.groupNames.has(name))) {
        rejectUnsupportedPattern("--grep-exclude", exclude, "a named backreference (e.g. \\k<name>)");
    }
    ...
    try {
        ...
        throw new HardhatError(HardhatError.ERRORS.HARDHAT_MOCHA.GENERAL
            .INVALID_GREP_EXCLUDE_COMBINATION, { name: includeName, grep: effectiveInclude ?? "", grepExclude: exclude });
    }
 ...
 */
function buildMergedGrep(include, exclude) {
    const includeAssertion = include === undefined ? "" : `(?=[\\s\\S]*(?:${include}))`;
...
function assertMergeableGrepPattern(name, pattern) {
    if (pattern !== undefined && MOCHA_REGEX_LITERAL.test(pattern)) {
        rejectUnsupportedPattern(name, pattern, "a /pattern/flags regex literal");
    }
}
function rejectUnsupportedPattern(name, pattern, feature) {
    throw new HardhatError(HardhatError.ERRORS.HARDHAT_MOCHA.GENERAL.GREP_EXCLUDE_UNSUPPORTED_PATTERN, { name, pattern, feature });
}
/**
 ...
 */
function analyzePattern(pattern, namedGroupsPresent) {
    const info = {
        capturingGroups: 0,
        hasNamedGroup: false,
...
function assertValidPattern(name, pattern) {
    ...
    try {
        ...
        throw new HardhatError(HardhatError.ERRORS.CORE.ARGUMENTS.INVALID_VALUE_FOR_TYPE, { value: pattern, name, type: "regexp" });
    }
}
//# sourceMappingURL=merge-grep.js.map