...
const perf = createPerformanceTracker(performanceScope, "Mocha test task");
...
async function getTestFiles(testFiles, config) {
    ...
    return await getAllFilesMatching(config.paths.tests.mocha, (f) => isJavascriptFile(f) || isTypescriptFile(f));
}
let testsAlreadyRun = false;
const testWithHardhat = async ({ testFiles, bail, grep, grepExclude, noCompile }, hre) => {
    ...
    const grepFilter = resolveMochaGrepFilter(grep, grepExclude, hre.config.test.mocha);
    perf.start();
    ...
    if (hre.config.test.mocha.parallel === true) {
        ...
        const unhandledRejectionHook = new URL(import.meta.resolve(unhandledRejectionHookPath));
        ...
        if (hre.globalOptions.coverage === true ||
            ...
            const testWorkerDone = new URL(import.meta.resolve("@nomicfoundation/hardhat-mocha/test-worker-done"));
    ...
    const mochaConfig = { ...hre.config.test.mocha };
    ...
    // in that case
    if (testsAlreadyRun) {
        throw new HardhatError(HardhatError.ERRORS.HARDHAT_MOCHA.GENERAL.TEST_TASK_ESM_TESTS_RUN_TWICE);
    }
    ...
    await hre.hooks.runHandlerChain("test", "onTestRunStart", ["mocha"], async () => { });
    let total = 0;
    ...
    if (hre.config.test.mocha.parallel !== true) {
        await hre.hooks.runHandlerChain("test", "onTestWorkerDone", ["mocha"], async () => { });
    }
    await hre.hooks.runHandlerChain("test", "onTestRunDone", ["mocha"], async () => { });
