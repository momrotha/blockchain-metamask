...
export class Blockscout {
    ...
    static async resolveConfig({ chainId, networkName, chainDescriptors, verificationProvidersConfig, dispatcher, shouldUseCache = true, }) {
        ...
        if (blockExplorerConfig === undefined) {
            ...
            throw new HardhatError(HardhatError.ERRORS.HARDHAT_VERIFY.GENERAL
    ...
    static async create({ blockExplorerConfig, verificationProviderConfig, dispatcher, }) {
    ...
    async verify({ contractAddress, compilerInput, contractName, compilerVersion, constructorArguments, }) {
        ...
        if (!blockscoutResponse.isOk()) {
            throw new HardhatError(HardhatError.ERRORS.HARDHAT_VERIFY.GENERAL
                .CONTRACT_VERIFICATION_REQUEST_FAILED, { message: blockscoutResponse.message });
        }
    ...
    async pollVerificationStatus(guid, contractAddress, contractName) {
        ...
        if (!blockscoutResponse.isOk()) {
            throw new HardhatError(HardhatError.ERRORS.HARDHAT_VERIFY.GENERAL
                .CONTRACT_VERIFICATION_STATUS_POLLING_FAILED, { message: blockscoutResponse.message });
        }
        if (!(blockscoutResponse.isFailure() || blockscoutResponse.isSuccess())) {
            // Reaching this point shouldn't be possible unless the API is behaving in a new way.
            throw new HardhatError(HardhatError.ERRORS.HARDHAT_VERIFY.GENERAL
                .CONTRACT_VERIFICATION_UNEXPECTED_RESPONSE, { message: blockscoutResponse.message });
        }
        return {
