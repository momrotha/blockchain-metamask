 ...
 */
export async function encodeConstructorArgs(abi, constructorArgs, contract) {
    ...
    try {
        ...
        return getUnprefixedHexString(contractInterface.encodeDeploy(constructorArgs));
    }
    catch (error) {
        ensureError(error);
...
function isInvalidConstructorArgsLengthError(error) {
...
function isInvalidConstructorArgTypeError(error) {
    return ("code" in error &&
        error.code === "INVALID_ARGUMENT" &&
        "argument" in error &&
...
function isConstructorArgOverflowError(error) {
    return ("code" in error &&
        error.code === "INVALID_ARGUMENT" &&
        "argument" in error &&
        typeof error.argument === "string" &&
