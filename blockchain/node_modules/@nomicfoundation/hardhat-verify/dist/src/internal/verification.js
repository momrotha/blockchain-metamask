...
import { filterVersionsByInferred, resolveSupportedSolcVersions, } from "./solc-versions.js";
 ...
 * );
 */
export async function verifyContract(verifyContractArgs, hre, consoleLog = console.log, dispatcher, provider) {
    const { artifacts, config, globalOptions: { buildProfile: buildProfileName = "production" }, network, solidity, } = hre;
    const { provider: verificationProviderName = ETHERSCAN_PROVIDER_NAME } = verifyContractArgs;
    validateVerificationProviderName(verificationProviderName);
    validateArgs(verifyContractArgs);
    const { address, constructorArgs = [], libraries = {}, contract, force = false, creationTxHash, } = verifyContractArgs;
    ...
    try {
        ...
        const isExplorerRequestError = HardhatError.isHardhatError(error, HardhatError.ERRORS.HARDHAT_VERIFY.GENERAL.EXPLORER_REQUEST_FAILED);
...
Explorer: ${instance.getContractUrl(address)}`);
    ...
    const deployedBytecode = await Bytecode.getDeployedContractBytecode(resolvedProvider, address, networkName);
    const compatibleSolcVersions = filterVersionsByInferred(supportedSolcVersions, deployedBytecode.solcVersion);
    if (compatibleSolcVersions.length === 0) {
        const configuredSolcVersionSummary = supportedSolcVersions.length > 1
            ? `versions are: ${supportedSolcVersions.join(", ")}`
            : `version is: ${supportedSolcVersions[0]}`;
        throw new HardhatError(HardhatError.ERRORS.HARDHAT_VERIFY.GENERAL.SOLC_VERSION_MISMATCH, {
            configuredSolcVersionSummary,
            deployedSolcVersion: formatInferredSolcVersion(deployedBytecode.solcVersion),
            networkName,
        });
    }
    const contractInformationResolver = new ContractInformationResolver(artifacts, compatibleSolcVersions, networkName);
    const contractInformation = await contractInformationResolver.resolve(contract, deployedBytecode);
    const libraryInformation = resolveLibraryInformation(contractInformation, libraries);
    const encodedConstructorArgs = await encodeConstructorArgs(contractInformation.compilerOutputContract.abi, constructorArgs, contractInformation.userFqn);
    const minimalCompilerInput = await getCompilerInput(solidity, config.paths.root, contractInformation.sourceName, contractInformation.inputFqn.startsWith("npm/"), buildProfileName);
    const { success: minimalInputVerificationSuccess, message: minimalInputMessage, isRetryable, } = await attemptVerification({
...
`);
        ...
        const { success: fullCompilerInputVerificationSuccess, message: fullInputMessage, } = await attemptVerification({
            verificationProvider: instance,
            address,
            encodedConstructorArgs,
            creationTxHash,
...
✅ Contract verified successfully on ${instance.name}!
  ...
  Explorer: ${instance.getContractUrl(address)}`);
    ...
    const librariesWarning = libraryInformation.undetectableLibraries.length > 0
        ? `
...
${libraryInformation.undetectableLibraries.map((x) => `  * ${x}`).join("\n")}`
        : "";
    throw new HardhatError(HardhatError.ERRORS.HARDHAT_VERIFY.GENERAL.CONTRACT_VERIFICATION_FAILED, {
        reason: failureReason,
        librariesWarning,
    });
}
export function validateVerificationProviderName(provider) {
    if (Object.keys(VERIFICATION_PROVIDERS).indexOf(String(provider)) === -1) {
        throw new HardhatError(HardhatError.ERRORS.HARDHAT_VERIFY.VALIDATION
            .INVALID_VERIFICATION_PROVIDER, {
            verificationProvider: String(provider),
            supportedVerificationProviders: Object.keys(VERIFICATION_PROVIDERS).join(", "),
        });
    }
}
export function validateArgs({ address, contract }) {
    if (!isAddress(address)) {
        throw new HardhatError(HardhatError.ERRORS.HARDHAT_VERIFY.VALIDATION.INVALID_ADDRESS, {
            value: address,
        });
    }
...
export async function createVerificationProviderInstance({ provider, networkName, chainDescriptors, verificationProviderName, verificationProvidersConfig, dispatcher, }) {
...
async function attemptVerification({ verificationProvider, address, encodedConstructorArgs, contractInformation, creationTxHash, }, consoleLog = console.log) {
...
`);
    ...
    const verificationStatus = await verificationProvider.pollVerificationStatus(guid, address, contractInformation.userFqn);
