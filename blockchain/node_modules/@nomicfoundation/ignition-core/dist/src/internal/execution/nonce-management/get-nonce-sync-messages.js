...
import { isArtifactContractAtFuture, isEncodeFunctionCallFuture, isNamedContractAtFuture, isReadEventArgumentFuture, } from "../../../type-guards.js";
 ...
 */
export async function getNonceSyncMessages(jsonRpcClient, deploymentState, ignitionModule, accounts, defaultSender, requiredConfirmations) {
    const messages = [];
    ...
    for (const [sender, pendingIgnitionTransactions] of Object.entries(pendingTransactionsPerSender)) {
        ...
        if (pendingIgnitionTransactions.length === 0) {
            if (hasOnchainUnconfirmedPendingTransactions) {
                throw new HardhatError(HardhatError.ERRORS.IGNITION.EXECUTION.WAITING_FOR_CONFIRMATIONS, {
                    sender,
        ...
        for (const { nonce, transactions, executionStateId, networkInteractionId, } of pendingIgnitionTransactions) {
            ...
            if (latestCount > nonce) {
                ...
                if (!hasEnoughConfirmations) {
                    throw new HardhatError(HardhatError.ERRORS.IGNITION.EXECUTION.WAITING_FOR_NONCE, {
                        sender,
                        nonce,
                        requiredConfirmations,
                    });
