 ...
 */
export class EIP1193JsonRpcClient {
    _provider;
    _config;
    ...
    async getNetworkFees() {
        ...
        if (this._config?.maxFeePerGasLimit !== undefined &&
            ...
            throw new HardhatError(HardhatError.ERRORS.IGNITION.EXECUTION
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes("base fee exceeds gas limit")) {
                    throw new HardhatError(HardhatError.ERRORS.IGNITION.EXECUTION.BASE_FEE_EXCEEDS_GAS_LIMIT);
                }
            }
            throw error;
        }
    ...
    async getFullTransaction(txHash) {
        ...
        return {
            ...
            blockNumber: response.blockNumber !== null
                ...
                : null,
            maxFeePerGas: "maxFeePerGas" in response
                ? jsonRpcQuantityToBigInt(response.maxFeePerGas)
                : undefined,
            maxPriorityFeePerGas: "maxPriorityFeePerGas" in response
                ? jsonRpcQuantityToBigInt(response.maxPriorityFeePerGas)
                : undefined,
            gasPrice: "gasPrice" in response
                ? jsonRpcQuantityToBigInt(response.gasPrice)
                : undefined,
            gasLimit: "gas" in response && response.gas !== undefined
    ...
    async getTransactionReceipt(txHash) {
        ...
        const contractAddress = status === TransactionReceiptStatus.SUCCESS
...
function assertResponseType(method, response, assertion) {
    if (!assertion) {
        throw new HardhatError(HardhatError.ERRORS.IGNITION.EXECUTION.INVALID_JSON_RPC_RESPONSE, {
            method,
            response: JSON.stringify(response),
        });
    }
}
function assertResponseIsNetworkTransactionType(response) {
    const method = "eth_getTransactionByHash";
    assertResponseType(method, response, typeof response === "object" && response !== null);
    ...
    assertResponseType(method, response, "input" in response && typeof response.input === "string");
    assertResponseType(method, response, "nonce" in response && typeof response.input === "string");
    if ("maxFeePerGas" in response) {
