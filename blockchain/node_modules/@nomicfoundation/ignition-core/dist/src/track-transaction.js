...
import { applyNewMessage, loadDeploymentState, } from "./internal/execution/deployment-state-helpers.js";
 ...
 * @public
 */
export async function trackTransaction(deploymentDir, txHash, provider, requiredConfirmations = defaultConfig.requiredConfirmations, applyNewMessageFn = applyNewMessage) {
    ...
    if (deploymentState === undefined) {
        throw new HardhatError(HardhatError.ERRORS.IGNITION.TRACK_TRANSACTIONS.UNINITIALIZED_DEPLOYMENT, {
            deploymentDir,
        });
    }
    ...
    if (transaction === undefined) {
        throw new HardhatError(HardhatError.ERRORS.IGNITION.TRACK_TRANSACTIONS.TRANSACTION_NOT_FOUND, {
            txHash,
        });
    }
            ...
            if (networkInteraction.type ===
                ...
                if (networkInteraction.transactions.length === 0) {
                    ...
                    if (networkInteraction.to?.toLowerCase() ===
                        ...
                        if ("maxFeePerGas" in transaction &&
                            ...
                            assertIgnitionInvariant("gasPrice" in transaction && transaction.gasPrice !== undefined, "Transaction fees are missing");
                            fees = {
                    ...
                    // so we check their transaction for the required number of confirmations
                        ...
                        return await checkConfirmations(exState, networkInteraction, transaction, requiredConfirmations, jsonRpcClient, deploymentState, deploymentLoader, applyNewMessageFn);
                    }
                }
                // case: the user gave us a transaction that matches a nonce we've already recorded sending from
                    ...
                    if (networkInteraction.transactions[0].hash === transaction.hash) {
                        throw new HardhatError(HardhatError.ERRORS.IGNITION.TRACK_TRANSACTIONS.KNOWN_TRANSACTION);
                    }
    ...
    throw new HardhatError(HardhatError.ERRORS.IGNITION.TRACK_TRANSACTIONS.MATCHING_NONCE_NOT_FOUND);
}
async function checkConfirmations(exState, networkInteraction, transaction, requiredConfirmations, jsonRpcClient, deploymentState, deploymentLoader, applyNewMessageFn) {
    ...
    assertIgnitionInvariant(receipt !== undefined, "Unable to retrieve transaction receipt");
...
If this is not the expected behavior, please edit your Hardhat Ignition module accordingly before re-running your deployment.`;
        ...
        throw new HardhatError(HardhatError.ERRORS.IGNITION.TRACK_TRANSACTIONS
